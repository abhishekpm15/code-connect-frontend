...
const Notification = () => {
  ...
  const handleViewProfile = (id) => {
    ...
    const handleAccept = (e, userId, notificationId,index,postID,sendToId,postName) => {
    ...
    const handleConfirm = () =>{
      ...
      console.log("user id , sender id , sendToId", userId, selectedUserId,sendToId,selectedPostName)
  ...
  return (
    <>
      <>
        ...
        <Modal title="📌 Rules and Regulations" open={isModalOpen2} onOk={handleOk2} onCancel={handleCancel2} width={"700px"} footer={null}>
            <div className="font-semibold ml-1" >Please read the below rules carefully ❗</div>
            <div className="flex flex-col space-y-3 mt-5">
              <div>• Upon mutual acceptance of the problem request, the problem/post will be moved to a restricted visibility state (only the developer involved can see the post).</div>
              <div>• If the developer is unable to solve the problem, you rerserve all rights to demote the person.</div>
              <div>• In the event of a conflict, the concerned party can email the platform, and appropriate actions will be taken accordingly.</div>
            </div>
            <div className="mt-5 font-bold text-center mb-10"> Collaboration is the cornerstone of innovation. <br></br> Respect the platform, and together, we can achieve greatness.</div>
            <div className="flex justify-center space-x-4">
              <Button  className="rounded-lg  bg-white text-black dark:border-gray-200 hover:bg-gray-100 border px-3 py-1" onClick={handleCancel2}>Decline</Button>
              ...
              { acceptLoad ?
                <Spin
                  ...
                  />
                  :
                  <Button  className="rounded-lg dark:bg-[#10172a] dark:text-white px-3 py-1" onClick={handleConfirm}>Confirm</Button>
              }
            </div>
        </Modal>
              ...
              <div className="px-10 py-5 w-full dark:bg-white rounded-xl dark:text-black text-white bg-[#10172a]">
               ...
               <span className="font-bold text-blue-500"> {notif?.notifications?.type === 'Interest' ? notif?.notifications?.message.split("\"")[1] : ""}</span>
                ...
                <div className="absolute space-x-2 z-50 btn-appear w-full text-center">
                  <Button  variant="outline" className=" rounded-lg  dark:text-white px-3 py-1" onClick={()=>{handleViewProfile(notif.userId)}}>View Profile</Button>
                  <Button variant="outline" className="rounded-lg w-30 dark:text-white px-2 py-1" onClick={(e)=>{handleAccept(e,notif.userId,notif._id,index,notif.notifications.postInfo.postID,notif.notifications.sentBy,notif.notifications.postInfo.postName)}}>Accept Interest</Button>
                </div>
              )}
              </div>
            </div>
